/*
 * This file is part of the GeoLatte project. GeoLatte is free software: you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free Software Foundation, either version 3 of
 * the License, or (at your option) any later version. GeoLatte is distributed in the hope that it will be useful, but
 ...
 * Public License along with GeoLatte. If not, see <http://www.gnu.org/licenses/>. Copyright (C) 2010 - 2011 and
 * Ownership of code is shared by: Qmino bvba - Romeinsestraat 18 - 3001 Heverlee (http://www.qmino.com) Geovise bvba -
...
public class JTS {
	...
	private static final PointSequenceCoordinateSequenceFactory pscsFactory =
			new PointSequenceCoordinateSequenceFactory();
	 ...
	 *
	 * @param geometryClass
	 *            the JTS Geometry class
	 * @return the corresponding o.g.geom class
	 * @throws IllegalArgumentException
	 *             when the geometryClass parameter is null.
	 * @throws NoSuchElementException
	 *             when no corresponding class can be found.
	 */
	public static Class<? extends Geometry>
			getCorrespondingJTSClass(Class<? extends org.geolatte.geom.Geometry> geometryClass) {
		...
		throw new NoSuchElementException(
				String.format("No mapping for class %s exists in JTS geom.", geometryClass.getCanonicalName()));
	}
	 ...
	 *            the Geolatte Geometry class
	 * @return the corresponding o.g.geom class
	 * @throws IllegalArgumentException
	 *             when the jtsGeometryClass parameter is null.
	 * @throws NoSuchElementException
	 *             when no corresponding class can be found.
	 */
	static Class<? extends org.geolatte.geom.Geometry>
			getCorrespondingGeolatteClass(Class<? extends Geometry> jtsGeometryClass) {
		if (jtsGeometryClass == null)
			throw new IllegalArgumentException("Null argument not allowed.");
		Class<? extends org.geolatte.geom.Geometry> corresponding = JTS2GLClassMap.get(jtsGeometryClass);
		if (corresponding == null) {
			throw new NoSuchElementException(
					String.format("No mapping for class %s exists in JTS geom.", jtsGeometryClass.getCanonicalName()));
		}
	 ...
	 *
	 * @param jtsGeometry
	 *            the jts geometry to convert
	 * @return an equivalent geolatte geometry
	 * @throws IllegalArgumentException
	 *             when a null object is passed
	 */
	...
	/**
	 * Factory method that converts a JTS geometry into an equivalent geolatte geometry and allows the caller to specify
	 * the CoordinateReferenceSystem of the resulting geolatte geometry.
	 *
	 * @param jtsGeometry
	 *            the jtsGeometry
	 * @param crs
	 ...
	 *
	 * @param geometry
	 *            the geolatte geometry to start from
	 * @param gFact
	 *            the GeometryFactory to use for creating the JTS Geometry
	...
	/**
	 * Converts a JTS <code>Envelope</code> to a geolatte <code>Envelope</code> with the specified CRS.
	 *
	 * @param jtsEnvelope
	 *            the JTS Envelope to convert.
	 * @param crs
	 *            the <code>CoordinateReferenceSystem</code> to use for the return value.
	 * @return the corresponding geolatte Envelope, having the CRS specified in the crsId parameter.
	 * @throws IllegalArgumentException
	 *             when a null object is passed
	 */
	public static <P extends Position> org.geolatte.geom.Envelope<P>
			from(com.vividsolutions.jts.geom.Envelope jtsEnvelope, CoordinateReferenceSystem<P> crs) {
		if (jtsEnvelope == null) {
			throw new IllegalArgumentException("Null object passed.");
		}
		return new org.geolatte.geom.Envelope<>(jtsEnvelope.getMinX(), jtsEnvelope.getMinY(), jtsEnvelope.getMaxX(),
	...
	public static com.vividsolutions.jts.geom.Envelope to(org.geolatte.geom.Envelope<?> env) {
		...
		return new com.vividsolutions.jts.geom.Envelope(env.lowerLeft().getCoordinate(0),
				env.upperRight().getCoordinate(0), env.lowerLeft().getCoordinate(1), env.upperRight().getCoordinate(1));
	}
	...
	private static <P extends Position> org.geolatte.geom.MultiPolygon<P> from(MultiPolygon jtsGeometry,
		...
		if (jtsGeometry.getNumGeometries() == 0)
			return new org.geolatte.geom.MultiPolygon<>(crs);
		org.geolatte.geom.Polygon<P>[] polygons = new org.geolatte.geom.Polygon[jtsGeometry.getNumGeometries()];
	...
	private static <P extends Position> org.geolatte.geom.Polygon<P> from(Polygon jtsGeometry,
		...
		if (jtsGeometry.isEmpty()) {
			return new org.geolatte.geom.Polygon<>(crs);
		}
		org.geolatte.geom.LinearRing<P>[] rings =
				new org.geolatte.geom.LinearRing[jtsGeometry.getNumInteriorRing() + 1];
	...
	private static <P extends Position> org.geolatte.geom.MultiLineString<P> from(MultiLineString jtsGeometry,
			CoordinateReferenceSystem<P> crs) {
		if (jtsGeometry.getNumGeometries() == 0)
			return new org.geolatte.geom.MultiLineString<>(crs);
		org.geolatte.geom.LineString<P>[] linestrings =
				new org.geolatte.geom.LineString[jtsGeometry.getNumGeometries()];
	 ...
	 */
	@SuppressWarnings("unchecked")
	private static <P extends Position> org.geolatte.geom.GeometryCollection<P, org.geolatte.geom.Geometry<P>>
			from(GeometryCollection jtsGeometry, CoordinateReferenceSystem<P> crs) {
		if (jtsGeometry.getNumGeometries() == 0)
	...
	private static <P extends Position> org.geolatte.geom.LineString<P> from(LineString jtsLineString,
		...
		return new org.geolatte.geom.LineString<>(pscsFactory.toPositionSequence(cs, crs.getPositionClass(), crs), crs);
	...
	private static <P extends Position> org.geolatte.geom.MultiPoint<P> from(MultiPoint jtsMultiPoint,
		...
		if (jtsMultiPoint == null || jtsMultiPoint.getNumGeometries() == 0)
			return new org.geolatte.geom.MultiPoint<>(crs);
	...
	private static <P extends Position> org.geolatte.geom.Point<P> from(com.vividsolutions.jts.geom.Point jtsPoint,
		...
		return new org.geolatte.geom.Point<>(pscsFactory.toPositionSequence(cs, crs.getPositionClass(), crs), crs);
	}
	...
	private static <P extends Position> Polygon to(org.geolatte.geom.Polygon<P> polygon, GeometryFactory gFact) {
		...
		Polygon jtsPolygon = gFact.createPolygon(shell, holes);
	...
	private static <P extends Position> GeometryCollection to(
			org.geolatte.geom.GeometryCollection<P, org.geolatte.geom.Geometry<P>> collection, GeometryFactory gFact) {
