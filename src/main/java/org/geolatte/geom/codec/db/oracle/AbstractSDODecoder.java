...
abstract public class AbstractSDODecoder implements Decoder<SDOGeometry> {
	...
	public Geometry<?> decode(SDOGeometry nativeGeom) {
		if (!accepts(nativeGeom)) {
			throw new IllegalArgumentException(
					getClass().getSimpleName() + " received object of type " + nativeGeom.getGType());
		}
	...
	protected <P extends Position> PositionSequence<P> convertOrdinateArray(Double[] oordinates, SDOGeometry sdoGeom,
	...
	protected <P extends Position> PositionSequence<P> add(PositionSequence<P> seq1, int seq1Offset,
			PositionSequence<P> seq2, int seq2Offset) {
		if (seq1 == null) {
			return seq2;
		}
		if (seq2 == null) {
		...
		CombiningVisitor<P> visitor = new CombiningVisitor<>(builder);
	...
	private <P extends Position> void addToBuilder(PositionSequence<P> seq, int skip,
			PositionSequenceBuilder<P> builder, CombiningVisitor<P> visitor) {
		if (skip == 0) {
			seq.accept(visitor);
		} else {
			for (P pos : seq) {
	...
	static class CombiningVisitor<P extends Position> implements LLAPositionVisitor {
		 ...
		 *
		 * @param coordinate
		 *            the visited coordinate in array representation
		 */
		@Override
	 ...
	 * @return
	 */
	protected <P extends Position> PositionSequence<P> getElementCSeq(int i, SDOGeometry sdoGeom, boolean hasNextSE,
			CoordinateReferenceSystem<P> crs) {
		...
		if (type.isStraightSegment()) {
			...
			cs = linearize(convertOrdinateArray(elemOrdinates, sdoGeom, crs), type.isCircle());
		} else if (type.isRect()) {
			cs = convertOrdinateArray(elemOrdinates, sdoGeom, crs);
	...
	private <P extends Position> PositionSequence<P> env2Seq(P p0, P p1, boolean asExteriorRing) {
		Envelope<P> env = new Envelope<>(p0, p1, (CoordinateReferenceSystem<P>) crs);
		if (asExteriorRing) {
			return fixedSized(5, (Class<P>) crs.getPositionClass()).add(env.lowerLeft()).add(env.lowerRight())
					.add(env.upperRight()).add(env.upperLeft()).add(env.lowerLeft()).toPositionSequence();
		} else {
			return fixedSized(5, (Class<P>) crs.getPositionClass()).add(env.lowerLeft()).add(env.upperLeft())
					.add(env.upperRight()).add(env.lowerRight()).add(env.lowerLeft()).toPositionSequence();
		}
	}
	...
	protected <P extends Position> PositionSequence<P> linearize(PositionSequence<P> positions, boolean entireCirlce) {
		...
		while (idx < positions.size() - 2) { // only iterate if we have at least three more points
			...
			CircularArcLinearizer<P> linearizer = new CircularArcLinearizer<>(p0, p1, p2, LINEARIZER_EPSILON);
